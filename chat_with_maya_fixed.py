#!/usr/bin/env python3
"""
Maya Chatbot Interface - FIXED Version for PyTorch 2.6+
"""
...
class MayaChatbot:
    def __init__(self, model_path):
        ...
        try:
            ...
            try:
                ...
                # Method 3: Recreate dataset from scratch
                print("⚠️ Recreating dataset...")
                checkpoint = self.recreate_checkpoint(model_path)
    ...
    def recreate_checkpoint(self, model_path):
        """Recreate checkpoint if loading fails"""
        # Load only model weights
        checkpoint_weights = torch.load(model_path, map_location='cpu')
        
        # Recreate dataset dengan data training yang sama
        from data.indonesian_data import IndonesianDataLoader
        data_loader = IndonesianDataLoader()
        indonesian_data, _ = data_loader.get_multiple_topics()
        
        dataset = TextDataset(text_data=indonesian_data, max_length=64)
        
        return {
            'model_state_dict': checkpoint_weights['model_state_dict'],
            'dataset': dataset
        }
...
def main():
    ...
    print("=" * 50)
    print("🧠 Model: 1.2 Juta Parameters!")
